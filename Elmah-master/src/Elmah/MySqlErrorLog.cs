...
namespace Elmah.MySql
{
    ...
    public class MySqlErrorLog : ErrorLog
    {
        ...
        public override string Log(Error error)
        {
            ...
            if (HttpContext.Current.Request.ServerVariables["HTTP_X_FORWARDED_FOR"] != null && HttpContext.Current.Request.ServerVariables["HTTP_X_FORWARDED_FOR"].ToString().Length > 5)
            {
                VisitorsIPAddress = HttpContext.Current.Request.ServerVariables["HTTP_X_FORWARDED_FOR"].ToString();
            }
            else if (HttpContext.Current.Request.ServerVariables["REMOTE_ADDR"] != null && HttpContext.Current.Request.ServerVariables["REMOTE_ADDR"].ToString().Length > 5)
            {
                VisitorsIPAddress = HttpContext.Current.Request.ServerVariables["REMOTE_ADDR"].ToString();
            }
            else if (HttpContext.Current.Request.UserHostAddress.Length != 0 && HttpContext.Current.Request.UserHostAddress.Length > 5)
            {
            ...
            using (var command = Commands.LogError(
                id, error.ApplicationName,
                error.HostName, error.Type.Substring(error.Type.LastIndexOf('.') + 1), 
                error.Source.Substring(error.Source.LastIndexOf('.') + 1), error.Message, error.User,
                error.StatusCode, error.Time.ToUniversalTime(), errorXml, VisitorsIPAddress, manufacturer))
            {
        ...
        private static class Commands
        {
                ...
                string manufacturer)
            {
                ...
                parameters.Add("VisitorsIPAddress", MySqlDbType.VarChar, 30).Value = visitorsIPAddress.Substring(0, Math.Min(30, visitorsIPAddress.Length));
                parameters.Add("Manufacturer", MySqlDbType.VarChar, 30).Value = manufacturer.Substring(0, Math.Min(30, manufacturer.Length));
