...
namespace Fintrak.Business.IFRS.Managers
{
    ...
    public class IFRSLoanManager : ManagerBase, IIFRSLoanService
    {
                ...
                using (TransactionScope ts = new TransactionScope())
                {
                    ...
                    if (module == null)
                    {
                        ...
                        var actionMenu = new systemCoreEntities.Menu()
                        {
                            ...
                            Alias = "Loan SetUp",
                            Action = "LOAN_SETUP",
                            ActionUrl = "ifrsloan-loansetup-list",
                            Image = null,
                        ...
                        actionMenu = new systemCoreEntities.Menu()
                        {
                            ...
                            ActionUrl = "ifrsloan-watchlistedloan-list",
                            Image = null,
                            ImageUrl = "action_image",
                            ModuleId = module.EntityId,
                            ParentId = root.EntityId,
        ...
        public void DeleteLoanSetupIdList(string loanSetupIdlist)
        {
            var connectionString = GetDataConnection();

            using (var con = new MySqlConnection(connectionString))
            {
                var cmd = new MySqlCommand("MultipleDeletion", con);
                cmd.CommandType = System.Data.CommandType.StoredProcedure;
                cmd.Parameters.Add(new MySqlParameter
                {
                    ParameterName = "@IdLists",
                    Value = loanSetupIdlist,
                });
                cmd.Parameters.Add(new MySqlParameter
                {
        ...
        public string[] GetRiskRatingCode()
        {
            return ExecuteFaultHandledOperation(() =>
            {
                ...
                IEnumerable<string> listOfcreditRiskCode = creditRiskRatingRepository.GetRiskRatingCode();

                return listOfcreditRiskCode.ToArray();
            });
        }
        #endregion
                ...
                foreach (var collateralRealizationPeriodInfo in collateralRealizationPeriodInfos)
                {
                    collateralRealizationPeriods.Add(
                        new CollateralRealizationPeriodData
                        {
                            ...
                            AvgRecoveryCost=collateralRealizationPeriodInfo.CollateralRealizationPeriod.AvgRecoveryCost,
                            HairCut=collateralRealizationPeriodInfo.CollateralRealizationPeriod.HairCut,
                            GrowthRate= collateralRealizationPeriodInfo.CollateralRealizationPeriod.GrowthRate,
                            Inflation = collateralRealizationPeriodInfo.CollateralRealizationPeriod.Inflation,
                            TimeToRecovery = collateralRealizationPeriodInfo.CollateralRealizationPeriod.TimeToRecovery,
                            Depreciation = collateralRealizationPeriodInfo.CollateralRealizationPeriod.Depreciation,
                ...
                foreach (var indSch in individualScheduleInfo)
                {
                    individualSchedule.Add(
                        new IndividualScheduleData
                        {
                            ...
                            PastDueAmount = indSch.PastDueAmount,
                            IRR = indSch.IRR,
                            FirstRepaymentdate = indSch.FirstRepaymentdate,
            ...
            return ExecuteFaultHandledOperation(() =>
            {
                ...
                foreach (var indSch in individualScheduleInfo)
                {
                    individualSchedule.Add(
                        new IndividualScheduleData
                        {
                            RefNo = indSch.LoanPryData.RefNo,
                            Amount = indSch.LoanPryData.Amount,
                            IRR = indSch.LoanPryData != null ? indSch.LoanPryData.Rate : 0,
                            FirstRepaymentdate = indSch.LoanPryData.FirstRepaymentdate,
                            MaturityDate = indSch.LoanPryData.MaturityDate,
                            Active = true,
                            Processed = false,
                            ValueDate = indSch.LoanPryData.ValueDate
                        });
                }
                return individualSchedule.ToArray();
            });
        ...
        public IndividualImpairmentData[] GetIndividualImpairments(string refNo)
        {
            return ExecuteFaultHandledOperation(() =>
            {
                ...
                IEnumerable<RawLoanDetails> individualImpairmentInfo = individualImpairmentRepository.GetIndividualImpairments(refNo).ToArray();
        ...
        public BondSummary UpdateBondSummary(BondSummary bondSummary)
        {
            return ExecuteFaultHandledOperation(() =>
            {
                ...
                IBondSummaryRepository bondSummaryRepository = _DataRepositoryFactory.GetDataRepository<IBondSummaryRepository>();

                BondSummary updatedEntity = null;

                if (bondSummary.BondId == 0)
                    updatedEntity = bondSummaryRepository.Add(bondSummary);
                else
                    updatedEntity = bondSummaryRepository.Update(bondSummary);
        ...
        public void DeleteBondSummary(int BondId)
        {
            ExecuteFaultHandledOperation(() =>
            {
                ...
                bondSummaryRepository.Remove(BondId);
            });
        }
        ...
        public BondSummary GetBondSummary(int BondId)
        {
            return ExecuteFaultHandledOperation(() =>
            {
                ...
                BondSummary bondSummaryEntity = bondSummaryRepository.Get(BondId);
                if (bondSummaryEntity == null)
                {
                    NotFoundException ex = new NotFoundException(string.Format("BondSummary with ID of {0} is not in database", BondId));
                    throw new FaultException<NotFoundException>(ex, ex.Message);
                }

                return bondSummaryEntity;
            });
        }
        ...
        public BondSummary[] GetAllBondSummarys()
        {
            return ExecuteFaultHandledOperation(() =>
            {
                ...
                IEnumerable<BondSummary> bondSummarys = bondSummaryRepository.Get().ToArray();

                return bondSummarys.ToArray();
            });
        }
        ...
        public string GetDataConnection()
        {
            ...
            string connectionString = "Data Source=.\fintrakMySql2014;Initial Catalog=FintrakDB;User =sa;Password=MySqluser10$;Integrated Security=False";
