    ...
    public class MySqlErrorLog : ErrorLog
    {
        ...
        public override string Log(Error error)
        {
            ...
            if (error.StatusCode == 0)
            {
                ...
                if (errorMessage.Contains("inner exception"))
                    error.StatusCode = 1000;
                if (errorMessage.Contains("Table"))
                    error.StatusCode = 3007;
                if (errorMessage.Contains("Input string"))
                    error.StatusCode = 7447;
                if (errorMessage.Contains("Bad Gateway"))
                    error.StatusCode = 502;
                if (errorMessage.Contains("rows (0). Entities"))
                    error.StatusCode = 3077;
                if (errorMessage.Contains("underlying provider failed"))
                    error.StatusCode = 1555;
            }

            using (var connection = new MySqlConnection(ConnectionString))
            using (var command = Commands.LogError(
                id, error.ApplicationName,
                error.HostName, error.Type.Substring(error.Type.IndexOf('.') + 1), 
