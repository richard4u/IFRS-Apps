...
(function () {
    ...
    function RunExtractionListController($scope, $state, $filter, viewModelHelper, validator, extractionService) {
        ...
        vm.infoLabel = '';
        vm.btnColour = '';
        vm.btnRefreshColour = 'btn-success';
        var exportTable1;
        var exportTable;
        ...
        var initialize = function () {
            ...
            if (vm.init === false) {
                ...
                vm.loadExtraction();
                vm.getServiceStatus();
                vm.viewModelHelper.apiGet('api/runextraction/getcurrentjobs',
                    null,
                    function (result) {
                ...
                vm.viewModelHelper.apiGet('api/solutionrundate/getsolutionrundatebyloginbydefault',
                    null,
                    //function (result) {
                    function (result) {
        ...
        var InitialGrid = function () {
            setTimeout(function () {
                ...
                if ($('#extractionTable').length > 0) {
                    exportTable = $('#extractionTable').DataTable({
        ...
        vm.runExtractions = function () {
            ...
            vm.viewModelHelper.apiPost(
                'api/runextraction/checkextraction/' + vm.startDate.toDateString() + '/' + vm.endDate.toDateString(),
                extractionIds,
                function (result) {
                    ...
                    if (result.data === '"Ok"') {
                        ...
                        var job = {
                            Code: '',
                            Status: 5,
                            StartDate: vm.startDate.toDateString(),
                            EndDate: vm.endDate.toDateString(),
                            RunTime: vm.runTime
                        };

                        vm.viewModelHelper.apiPost('api/runextraction/updateextractionjob',
                            job,
                            function (result) {
                                job = result.data;
                                vm.runTime = new Date(1970, 0, 1, 14, 57, 0);
                                vm.viewModelHelper.apiPost(
                                    'api/runextraction/startextraction/' +
                                    job.ExtractionJobId +
                                    '/' +
        ...
        vm.cancelExtractions = function () {
            vm.viewModelHelper.apiGet(
                'api/runextraction/cancelextractionjob/' +
                vm.code +
                '/' +
                vm.startDate.toDateString() +
                '/' +
        ...
        vm.loadJobs = function () {
            vm.viewModelHelper.apiGet(
                'api/runextraction/getjobs/' + vm.startDate.toDateString() + '/' + vm.endDate.toDateString(),
                null,
                function (result) {
                    vm.extractionJobs = result.data;
                    vm.code = '';
        ...
        vm.setAutoRefresh = function () {
            ...
            if (!enableAutoRefresh) {
                vm.autoRefresh = 'Disable Refresh';
                vm.btnRefreshColour = 'btn-danger';
                ...
                function (result) {
                    ...
                    switch (vm.servicestatus) {
                        ...
                        case '"Stopped"':
                            ...
                            toastr.info('Please contact IT/support to start \"' + serviceName + '\" on Fintrak server.</br> Thereafter, Click the ' + vm.servicestatus + ' button.', 'Service ' + vm.servicestatus, { timeOut: 0, extendedTimeOut: 0, closeButton: true, tapToDismiss: false, preventDuplicates: true });
                            break;
                        case '"Paused"':
                            vm.btnColour = 'btn-warning button1';
                            toastr.info('Please contact IT/support to restart \"' + serviceName + '\" on Fintrak server.</br> Thereafter, Click the ' + vm.servicestatus + ' button.', 'Service ' + vm.servicestatus, { timeOut: 0, extendedTimeOut: 0, closeButton: true, tapToDismiss: false, preventDuplicates: true });
                            break;
                        case '"Starting"':
                        ...
                        case '"Service Not Yet Installed"':
                            ...
                            toastr.info('Please contact IT/support. \"' + serviceName + '\" is not installed on Fintrak server', vm.servicestatus, { timeOut: 0, extendedTimeOut: 0, closeButton: true, tapToDismiss: false, preventDuplicates: true, preventOpenDuplicates: true });
                            break;
                        default:
                            vm.btnColour = 'btn-custom-secondary';
                            toastr.info('Please contact IT/support to restart \"' + serviceName + '\" on Fintrak server.</br> Thereafter, Click the ' + vm.servicestatus + ' button.', vm.servicestatus, { timeOut: 0, extendedTimeOut: 0, closeButton: true, tapToDismiss: false, preventDuplicates: true });
                    }
