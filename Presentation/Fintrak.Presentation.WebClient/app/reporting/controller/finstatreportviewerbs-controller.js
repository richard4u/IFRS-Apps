...
(function () {
    ...
    angular
        .module("fintrak")
        .controller("FinstatReportViewerController",
            ['$scope', '$window', '$state', '$stateParams', 'viewModelHelper', 'validator',
                FinstatReportViewerController]);

    function FinstatReportViewerController($scope, $window, $state, $stateParams, viewModelHelper, validator) {
        var vm = this;
        vm.viewModelHelper = viewModelHelper;
        vm.parentController = $scope.$parent;

        vm.module = 'Finstat';
        vm.view = 'finstat-reportviewer-view';
        vm.viewName = 'FinstatReportViewer';
        ...
        vm.checkinputLength = function () {
            ...
            if (inputrunDate.value.length >= 9 && inputCurrency.value.length >= 1 && inputbudgetType.value.length >= 1) {
                var stat = true;
                return stat;
            }
            else {
                var stat = false;
                alert('Please ensure the Report Date, Currency and Budget Type are not Empty');
                inputrunDate.focus;
                inputCurrency.focus;
                inputbudgetType.focus;
                return stat;
            }
