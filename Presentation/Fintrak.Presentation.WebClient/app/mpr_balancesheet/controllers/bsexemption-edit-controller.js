    ...
    function BSExemptionEditController($scope,$window, $state, $stateParams, viewModelHelper, validator) {
            ...
            if (vm.init === false) {
                ...
                if ($stateParams.bsexemptionId !== 0) {
                   ...
                   function (result) {
                       ...
                       initialView();
                       vm.init === true;
                       toastr.success('Selected item data loaded, ready for modifiation.', 'Fintrak');
                   },
                   function (result) {
                       toastr.error('Fail to load selected item', 'Fintrak');
                   }, null);
               }
               else
        ...
        vm.save = function () {
            ...
            if (vm.viewModelHelper.modelIsValid) {
               ...
               function (result) {
                   toastr.success('Item save successfully.', 'Fintrak');
                   $state.go('mpr-bsexemption-list');
               },
               function (result) {
                   toastr.error('Fail to save item', 'Fintrak');
               }, null);
            }
            else
            {
