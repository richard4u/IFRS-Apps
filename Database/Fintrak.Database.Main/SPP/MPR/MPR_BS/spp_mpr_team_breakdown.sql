...
Create proc [dbo].[spp_MPR_Team_Breakdown]
...
--select top 1 a.DefinitionCode, a.Code into level_exception_temp from mpr_team a where a.Code in 
--(select b.TeamCode from mpr_balancesheet b);
...
and a.Year = b.Year  where a.Year = Year(@rundate) and a.Active = 1 and a.Deleted = 0 and b.Active = 1 and b.Deleted = 0 
and a.DefinitionCode not in (select 'ACCT' union select 'TEM')
and a.Code in 
(select c.ParentCode from mpr_team c where c.Year = Year(@rundate) and c.Active = 1 and c.Deleted = 0 )) v;
...
Begin
	...
	select code , ParentCode  from mpr_team a where ParentCode = @caption and a.Year = Year(@rundate) and a.Active = 1 and a.Deleted = 0 ;
	
	insert into team_mapping(mis, parentMIS)
	SELECT   a.MIS, b.ParentCode
FROM      team_mapping a inner join mpr_team b on a.parentMIS = b.Code 
where b.ParentCode = @caption and b.Year = Year(@rundate) and b.Active = 1 and b.Deleted = 0 ;
