...
namespace MySql.Data.MySqlClient.Tests
{
  public class MySqlConnectionTests : BaseFixture
  {
    public override void SetFixture(SetUpClassPerTestInit fixture)
    {
      ...
      if (_fixture.conn.State != ConnectionState.Open && !_fixture.conn.SoftClosed)
        _fixture.conn.Open();
    }
    ...
    public void ConnectUsingMySqlNativePasswordPlugin()
    {
      ...
      MySqlConnectionStringBuilder Settings = new MySqlConnectionStringBuilder(_fixture.conn.Settings.ConnectionString);
      Settings.UserID = userName;
      Settings.Password = password;
      _fixture.CreateUser(userName, password, pluginName);
    ...
    public void ConnectUsingSha256PasswordPlugin()
    {
      ...
      Settings.UserID = userName;
      Settings.Password = password;
      Settings.Server = _fixture.conn.Settings.Server;
      Settings.Port = _fixture.conn.Settings.Port;
    ...
    {
      if (_fixture.Version < new Version(8, 0, 3)) return;

      MySqlDataReader pluginReader = _fixture.execReader("SELECT * FROM INFORMATION_SCHEMA.PLUGINS WHERE PLUGIN_NAME = 'caching_sha2_password'");
      ...
      MySqlConnectionStringBuilder builder = new MySqlConnectionStringBuilder(_fixture.conn.Settings.ConnectionString);
      builder.UserID = "testCachingSha2";
      builder.Password = "test";
      _fixture.CreateUser(builder.UserID, builder.Password, pluginName);
      ...
      _fixture.ExecuteSQLAsRoot("FLUSH PRIVILEGES");
      using (MySqlConnection connection = new MySqlConnection(builder.ConnectionString))
      {
        connection.Open();
        Assert.Equal(AuthStage.FULL_AUTH, CachingSha2AuthenticationPlugin._authStage);
    ...
    public void AllowPublicKeyRetrievalForSha256PasswordPlugin()
    {
      ...
      using (MySqlConnection connection = new MySqlConnection(Settings.ConnectionString))
      {
        ...
        if (serverCompiledUsingOpenSsl)
          ...
          Assert.Contains("Authentication to host", ex.Message);
      }
    ...
    {
      string connectionString = _fixture.GetConnectionString(false);
    ...
    public void EmptyPasswordOnMySqlNativePassword()
    {
      ...
      builder.Password = "";
      builder.Server = _fixture.conn.Settings.Server;
      builder.Port = _fixture.conn.Settings.Port;
      _fixture.CreateUser(builder.UserID, builder.Password, pluginName);
